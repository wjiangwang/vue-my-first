<template>
  <div class="page management-manoeuvres">
    <breadcrumb :items="[{ name: '应急救援演练' }]" v-show="breadcrumbShow"></breadcrumb>
    <router-view/>
    <div class="page-main">
      <page-card :scrollable="false" v-loading="list.loading">
        ...
        >
          ...
          <div slot="actions" slot-scope="{ row }" class="table-actions">
            <el-button type="text" size="medium" :readonly="true" @click="detail(row)">查看企业演练</el-button>
          </div>
        </data-table>
      </page-card>
    </div>
...
export default {
  ...
  computed: {
    ...
    tableColumns () {
      return [
        ...
        { title: '计划', name: 'exercisePlanNumber', renderHeader: this.renderHeaderPlan },
        { title: '记录', name: 'exerciseRecordNumber', renderHeader: this.renderHeaderRecord },
        { title: '总结', name: 'exerciseConclusionNumber', renderHeader: this.renderHeaderSummary },
  ...
  methods: {
    ...
    ...mapActions('managementContingencyPlan', ['filterData', 'getData']),
    ...
    detail (row) {
      this.$router.push({ name: 'emergency-drill-plan', params: { enterpriseId: row.id, enterpriseName: row.enterpriseName } })

    },

    renderHeaderPlan (h, { column, $index }, index) {
      return (
        <span>计划
          <span class="sum-num"> （{this.totalNumber.totalExercisePlanNumber}） </span>
        </span>
      )
    },

    renderHeaderRecord (h, { column, $index }, index) {
      return (
        <span>记录
          <span class="sum-num"> （{this.totalNumber.totalExerciseRecordNumber}） </span>
        </span>
      )
    },

    renderHeaderSummary (h, { column, $index }, index) {
      return (
        <span>总结
          <span class="sum-num"> （{this.totalNumber.totalExerciseConclusionNumber}） </span>
        </span>
      )
    },
  },
  ...
  beforeRouteUpdate (to, from, next) {
    this.breadcrumbShow = to.params.enterpriseId ? false : true
    next()
  },
