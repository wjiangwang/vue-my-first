...
export default {
  ...
  mutations: {
    ...
    setPlanarGraphData(state, data) {
      ...
      Object.assign(state.planarGraph, data.map || {
        id: '',
        url: ''
      })
    },
    ...
    selectRisk(state, risk) {
      ...
      const markupIndex = state.markups.findIndex(m => m.riskAssessmentId === risk.id)
      if (markupIndex !== -1) {
  ...
  actions: {
    async loadData({ state, commit }) {
      ...
      const result = await request.get(
        `/api/v2/elementArgument/${state.enterpriseId}`
      )
    ...
    async savePlanarGraph({ state, commit }, formData) {
      let requestURL = '/api/v2/elementArgument/map'
      let method = 'post'

      if (state.planarGraph.id) {
        requestURL = `${requestURL}/${state.planarGraph.id}`
        method = 'put'
      }
      ...
      if (state.planarGraph.id)
        commit('setPlanarGraphURL', formData)
      else
        commit('setPlanarGraphURL', { ...formData, id: result })
    },

    async createMarkup({ state, commit }, data) {
      const result = await request.post(
        '/api/v2/elementArgument',
        Object.assign({ mapId: state.planarGraph.id }, data)
      )

      commit('addMarkup', Object.assign({ id: result }, {
        x: data.left,
        y: data.top,
        w: data.width,
        h: data.height,
